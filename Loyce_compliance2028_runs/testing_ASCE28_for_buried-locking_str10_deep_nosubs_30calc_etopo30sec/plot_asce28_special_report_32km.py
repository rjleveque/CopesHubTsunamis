...
for gaugeno in range(gstart,gend+1):
    ...
    #
    ####  Even this can be wrong if eta[0] not zero:
    #B0 = -h[0]     #the first h in the file was really h0, pre-quake, all ASCE
                   #gauges are in the water

    ### Safer way.  Use B[-1] for B, then use dzi_gauge to get B0  (or separate run for B0).
    ### Here B[-1] and Bpost_quake_finest are equal.  These gauges are in a fixed region and
    ### all always wet, so no problems.
    B0 = B[-1] - dzi_gauge
...
if 1:  #Print report for all the gauges from gstart to gend. Thinking of each gauge  as a structure, try to find the
    ...
    print (' ')
    print ('gauge_no,   latitude,   longitude,   GeoClaw_eta,    ASCE_eta,     0.8*(ASCE_eta),  .8*ASCE/Geo,  g_N,  g_S,   del_km,  ASCE_mean,  GeoClaw_mean,  Compliant,  mean_ratio')
    for gaugeno in range(gstart,gend+1):  
        ## Now print 
        key = str(gaugeno)
        geoc=geo_amp[gaugeno-1]
        ...
        ##print the ratio of the means mean_asce/mean_geoc
        asce_to_geoc_mean = mean_asce/mean_geoc

        print ('%8i, %11.6f, %11.6f, %9.3f, %13.3f, %13.3f, %15.3f, %6i, %5i, %8.3f, %8.3f, %10.3f, %10s, %11.3f' %(gaugeno,ascee_lat,ascee_long,\
                geoc,ascee,ascee_pt8,ratio3,g_32N,g_32S,del_km,mean_asce,mean_geoc,gauge_is_compliant,asce_to_geoc_mean))
    ...
    for i in range(num_compliant_gaugenos):
        print('%8.1f %16.6f %15.6f'  %(compliant_gaugenos[i],compliant_lon[i],compliant_lat[i]) )
