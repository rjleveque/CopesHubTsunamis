...
def set_slip(fault0, event):
    ...
    if rupture_type == 'kinematic':
        fname = f'rupt_time_resampled_source_saved_{event_jey}.out'
        rupt_time = loadtxt(os.path.join(event_dir, fname))[:,2]
        fname = f'rise_time_resampled_source_saved_{event_jey}.out'
    ...
    for Nfault in frontal_faults:
        nsubfaults = faults_triangles[Nfault].shape[0]

        fname = f'mag_slip_resampled_source_{event_dash}_{Nfault}.out'
        mag_slip = loadtxt(os.path.join(event_dir, fname))[:,2]
