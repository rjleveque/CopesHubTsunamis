...
def load_fgmax(outdir,fgno,fname_B0):
    ...
    try:
        ...
        try:
            t_hours = min(clawdata.output_times[-1], fg.tend_max) / 3600.
        except:
            t_hours = nan
...
def make_fgmax_plots(fg, fgmax_plotdir, run_name, t_hours, GE_image, GE_extent):
    ...
    def annotate_transect(axtrans):
        ...
        #             ha='center',va='top')
        axtrans.plot([x2trans-1.5*dxlong,x2trans-dxlong/2], [-12,-12],'k')
        axtrans.text(x2trans-dxlong/2, -13, '%i km' % dxkm, \
                     ha='center',va='top')
        axtrans.grid(True)
...
if __name__== '__main__':
    ...
    graphics_dir = os.path.join(CHT, 'geoclaw_runs/sites/WestportCoast')
    B0_dir = os.path.join(CHT, 'geoclaw_runs/sites/WestportCoast')

    ###########
    #Choose the fgnos to use
    fgnos = [1]
    image_names = [graphics_dir + '/GE_WestportCoast.jpg']
    GE_extents = [[-124.165, -124.06, 46.75, 46.92]]
    fnames_B0 = [B0_dir + '/WestportCoast_B0.asc']
    ##########
