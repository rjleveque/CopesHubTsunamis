...
if 1:
    ...
    event = os.path.split(os.getcwd())[-1].replace('_bouss','')
...
#------------------------------
    ...
    dtopo_dir = os.path.join(root_dir, 'dtopo/CSZ_groundmotions/vertical_displacements_sensitivity')
    dtopo_data.dtopofiles = [[3, dtopo_dir + '/ft-random-mur13-deep_instant.dtt3']]
    ...
    fgout.x1 = -128.  # specify edges (fgout pts will be cell centers)
    fgout.x2 = -122.
    fgout.y1 = 44.5
    fgout.y2 = 47.5
    ...
    if 0:
        # small region around Seaside
        dx_fgout = 1/3600.  # degrees
        dy_fgout = 1/3600.  # degrees
        dt_fgout = 15  # seconds
        fgout = fgout_tools.FGoutGrid()
        fgout.fgno = 2
        fgout.point_style = 2       # will specify a 2d grid of points
        fgout.output_format = 'binary32'
        fgout.x1 = -124.0  # specify edges (fgout pts will be cell centers)
        fgout.x2 = -123.9
        fgout.y1 = 45.97
        fgout.y2 = 46.02
        fgout.nx = int(round((fgout.x2 - fgout.x1)/dx_fgout))
