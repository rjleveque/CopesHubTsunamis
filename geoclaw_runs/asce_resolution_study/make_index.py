...
with open(fname_html,'w') as f:
    f.write('<html>\n<h1>Gauge plots for event buried-locking-str10-deep</h1>\n')
    timenow = datetime.datetime.today().strftime('%Y-%m-%d at %H:%M:%S')
    f.write('<h3>Generated %s</h3>\n' % timenow)
    
    f.write("""<h2>Tsunami propagation</h2>\n
This <a href="geoclaw_buried-locking-str10-deep_dual.mp4">animation</a>
shows the tsunami propagation, both for the time-dependent ruputure (on the left)
and for an instantaneous rupture based on the final displacement (on the right).
The time-dependent version was used for the plots below.
<p>
...
different events or time-dependent vs. instantaneous.\n""")
