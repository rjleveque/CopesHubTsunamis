...
def load_fgmax(outdir,fgno,fname_B0=None):
    ...
    #fg.read_output(outdir=outdir, indexing='ij')  # so fg.h[i,j] for xi,yj
    ...
    if fname_B0:
        ...
        # assume it is an npy file
        #B0 = load(fname_B0).T
        #B0_masked = ma.masked_array(B0, fg.B.mask)
        #fg.B0 = B0_masked
        print('Loading B0 from ',fname_B0)
    else:
        fg.B0 = fg.B
...
def make_all_fgmax_plots(outdir, plotdir, location, event):
    ...
    for fgno in [1]:
        ...
        #fname_B0 = '../GH3s_B0/GH3s_fgmax1_B0.npy'
