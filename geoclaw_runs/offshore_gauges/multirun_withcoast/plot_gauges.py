...
def make_gauge_plot(gaugenos, outdir, plotdir, location, event):
    ...
    if plot_eta:
        ...
        try:
            ...
            fgout_grid.read_fgout_grids_data_pre511()  # in case older
    ...
    ax2.plot(ampl,yg,'b-', label='instantaneous')
    if plot_line:
    ...
    ax2.grid(True)
    #ax2.set_xlim(14,0)
    #ax2.set_xlim(24,0)  # modified for ft_ model
    ampmax = max(ampl.max(), 10)
