...
#------------------------------
    ...
    if clawdata.output_style==1:
        ...
        ## ADJUST:
        clawdata.num_output_times = 12    #output every 5 minutes
        #clawdata.tfinal = 2*3600.
        clawdata.tfinal = 3600.           #run for 1 hour
    ...
    amrdata.amr_levels_max = 7      #1/3 arc second
    ...
    #Need something made coarser between 1sec and 5sec data out to the inner RR region
    ...
    # Continential shelf extended to cover dtopo, 10"
    flagregion = FlagRegion(num_dim=2)
    ...
    # Continential shelf Variable Region, 30" to 10"
    flagregion = FlagRegion(num_dim=2)
    ...
    if 0: #For 10" run around region of interest (This was my slider window).
        # Rectangular region that encompasses gauges 94-137, offshore OSVES
        # (or gauges 98-143, offshore Westport).
        # Make this region 10" for all time thinking gauge plots will be better
    ...
    if 0: ### Probably NEVER use  
        #For the 5" runs (Trying to keep this at 10" from the coastal region b)
    ...
    if 1: ### Will use this for the inundation runs. (5" slider window)
        # Rectangular region that encompasses gauges 94-137, offshore OSVES
        # or gauges 98-143, offshore Westport.
    ...
    if 1: #will need to fix the regions below for each collaboratory
          # dx = dy = 4', 1', 30", 10", 5", 1", 1/3", 1/9"
    
        # Westport Variable Region - 30sec to 10sec to 5sec:
        flagregion = FlagRegion(num_dim=2)
        flagregion.name = 'Region_Westport_30-10-5sec'
        flagregion.minlevel = 3
        flagregion.maxlevel = 5
        flagregion.t1 = 0.
        flagregion.t2 = 1e9
        ...
        # Grays Harbor Region - allow  1" grids, require 10sec at least:
        flagregion = FlagRegion(num_dim=2)
        flagregion.name = 'Region_Grays_10-5-1sec'
        flagregion.minlevel = 4
        flagregion.maxlevel = 6
        flagregion.t1 = tstart_finestgrid
        flagregion.t2 = 1e9
        flagregion.spatial_region_type = 1  # Rectangle
        flagregion.spatial_region = [-124.199, -123.809, 46.785, 47.145]
        flagregions.append(flagregion)
        ...
        flagregion.t2 = 1e9
        flagregion.spatial_region_type = 1  # Rectangle
        flagregion.spatial_region = [-124.185,-123.935,46.8,46.935]
        flagregions.append(flagregion)
        ...
        if 1:
            ...
            flagregion.t2 = 1e9
            flagregion.spatial_region_type = 1  # Rectangle
            flagregion.spatial_region = [-124.13,-124.1,46.9,46.915]
            flagregions.append(flagregion)
    ...
    if 1:
        #Gauges around Westport for the inundation run, say for a 1/3" run,
        #make these gauges be a the domain cell centers.
        one_sixth = 1.0/(3600.*6); one_third = 1.0/(3600.*3)
        c_long = -124. - one_sixth; gx = c_long - 1269*one_third;
        c_lat = 46. + one_sixth; gy = c_lat + 9844*one_third;
        gaugeno = 1; #Land point at Westport Viewing Tower
        rundata.gaugedata.gauges.append([gaugeno, gx, gy, 0, 1e9]) 

        c_long = -124. - one_sixth; gx = c_long - 1188*one_third;
        c_lat = 46. + one_sixth; gy = c_lat + 9823*one_third;
        gaugeno = 2; #Water point in Westport Marina
        rundata.gaugedata.gauges.append([gaugeno, gx, gy, 0, 1e9]) 

    if 0:
        #Dont have asce gauges for the inundation runs
    ...
    if 0:
        #Don't use fgmax grid over whole domain at coarser level yet.
    ...
    if 0:
        ...
        fgout.nx = 8*60
        fgout.ny = 12*60
        fgout.x1 = -130.   # specify edges 
        fgout.x2 = -122.   # edge of a cell
    ...
    if 0: 
        #extra one for editing, numbers below are NOT yet edges of a cell in current domain
        fgout = fgout_tools.FGoutGrid()
        fgout.fgno = 2
        fgout.point_style = 2       # will specify a 2d grid of points
        fgout.output_format = 'binary32'
