...
#------------------------------
    ...
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.295,-123.85,46.76,47.12]
    flagregions.append(flagregion)
    ...
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.245,-124.02, 46.855,47.06]
    flagregions.append(flagregion)

    # 1/3 sec region
    # Level 8 is 1 sec
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_OceanShores'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.185,-124.1225, 46.93,47.0206]
    flagregions.append(flagregion)

    if 0:  #used to help make the area where the canals are for set eta init. Not
           #to be a computational region
            # Testing a Ruled Rectangle to encompass the canals
            # Level 8 is 1/3 sec
            flagregion = FlagRegion(num_dim=2)
            flagregion.name = 'Region_canals'
            flagregion.minlevel = 8
            flagregion.maxlevel = 8
            flagregion.t1 = 0.
            flagregion.t2 = 1e9
            flagregion.spatial_region_type = 2  # Ruled Rectangle
            flagregion.spatial_region_file = os.path.abspath('RuledRectangle_canals.data')
            flagregions.append(flagregion)
            slu_canals=np.array([[46.9472,-124.141,-124.1323],\
                                [46.9519,-124.168,-124.1323],\
                                [46.952,-124.168,-124.1323],\
                                [46.9752,-124.168,-124.1418],\
                                [46.9914,-124.168,-124.1418],\
                                [47.005648,-124.168,-124.1483],\
                                [47.012,-124.168,-124.1483],\
                                [47.0206,-124.168,-124.1537]])
            rr_canals = region_tools.RuledRectangle(slu=slu_canals)
            rr_canals.ixy = 'y'     #s refers to y, the lat, lower and upper in long x
            rr_canals.method = 1  #connect linearly
            rr_canals_name = 'RuledRectangle_canals'
            rr_canals.write(rr_canals_name + '.data')
    ...
    gauges = []
    if 1:
        gauges.append([154, -124.15240741, 46.96342593, 0, 1e+09])
        gauges.append([365, -124.14611111, 46.98888889, 0, 1e+09])
        gauges.append([366, -124.14851852, 46.99805556, 0, 1e+09])
        gauges.append([370, -124.13500000, 46.95259259, 0, 1e+09])
        gauges.append([408, -124.15657407, 46.98462963, 0, 1e+09])
        gauges.append([409, -124.14990741, 46.98527778, 0, 1e+09])
        gauges.append([431, -124.14666667, 46.95148148, 0, 1e+09])
        gauges.append([458, -124.15759259, 46.95990741, 0, 1e+09])
        gauges.append([483, -124.15055556, 47.00564815, 0, 1e+09])
        gauges.append([544, -124.15935185, 46.99981481, 0, 1e+09])
