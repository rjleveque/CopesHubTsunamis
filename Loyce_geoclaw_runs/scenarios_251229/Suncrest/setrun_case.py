...
#------------------------------
    ...
    if clawdata.output_style==1:
        ...
        clawdata.tfinal = 8.     #test run for 10 seconds 
    ...
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-126.6,-124.0,46.5,47.6185]
    flagregions.append(flagregion)
    ...
    gauges_region = [-125.9,-124.0,46.75,47.3]
    ...
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.295,-124.145,46.9,47.227]
    flagregions.append(flagregion)
    ...
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.245, -124.15175, 47.032,47.149]
    flagregions.append(flagregion)
   
    # 1/3 sec region
    # Level 8 is 1 sec
    #516 in north to 541 in south, north of Ocean Shores
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_Suncrest'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.1875,-124.1625, 47.053,47.093]
    flagregions.append(flagregion)

    # 1/3 sec region
    # Level 8 is 1/3 sec
    #River entrance, then 521 to 517  south, all north of Suncrest
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_Copalis1'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.19,-124.175, 47.093,47.124]
    flagregions.append(flagregion)

    # Copalis River and Ghost Forest
    # Level 8 is 1/3 sec
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_Copalis2'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 2  # Ruled Rectangle
    flagregion.spatial_region_file = os.path.abspath('RuledRectangle_Copalis2.data')
    flagregions.append(flagregion)
    slu_Cop2=np.array([[-124.193,47.1143,47.143],[-124.175,47.1143,47.143],[-124.15175,47.1143,47.1217]])
    rr_Cop2 = region_tools.RuledRectangle(slu=slu_Cop2)
    rr_Cop2.ixy = 'x'     #s refers to x, the longitude, lower and upper in lat y
    rr_Cop2.method = 1  #connect linearly
    rr_Cop2_name = 'RuledRectangle_Copalis2'
    rr_Cop2.write(rr_Cop2_name + '.data')
    ...
    #For Bridges
    gauges = []
    if 1: #from Suncrest region
        gauges.append([76, -124.16333333, 47.05601852, 0, 1e+09])
        gauges.append([218, -124.16777778, 47.07101852, 0, 1e+09])
        gauges.append([512, -124.16916667, 47.07722222, 0, 1e+09])
        gauges.append([513, -124.16972222, 47.07935185, 0, 1e+09])
        gauges.append([514, -124.17009259, 47.08101852, 0, 1e+09])
        gauges.append([515, -124.17166667, 47.08333333, 0, 1e+09])
        gauges.append([516, -124.17231481, 47.08648148, 0, 1e+09])
        gauges.append([541, -124.16462963, 47.05472222, 0, 1e+09])

    if 1: #from Copalis1 region
        gauges.append([517, -124.17712963, 47.09611111, 0, 1e+09])
        gauges.append([518, -124.17750000, 47.09814815, 0, 1e+09])
        gauges.append([519, -124.17796296, 47.10268519, 0, 1e+09])
        gauges.append([520, -124.17870370, 47.10462963, 0, 1e+09])
        gauges.append([521, -124.18027778, 47.11129630, 0, 1e+09])

    if 1: #from Copalis2 region
        gauges.append([62, -124.16953704, 47.11666667, 0, 1e+09])
        gauges.append([439, -124.15203704, 47.12101852, 0, 1e+09])
