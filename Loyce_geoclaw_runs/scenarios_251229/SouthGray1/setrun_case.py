...
#------------------------------
    ...
    if clawdata.output_style==1:
        ...
        #clawdata.tfinal = 10        #test run for 10 sec on laptop 
    ...
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.295,-123.75,46.6,47.145]
    flagregions.append(flagregion)

    # 3sec to 1sec region a
    # Level 7 is 1 sec
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_3sec_1sec_a'
    flagregion.minlevel = 6
    flagregion.maxlevel = 7
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.245,-123.93, 46.8025,46.985]
    flagregions.append(flagregion)

    # 3sec to 1sec region b
    # Level 7 is 1 sec
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_3sec_1sec_b'
    flagregion.minlevel = 6
    flagregion.maxlevel = 7
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-123.93,-123.850, 46.93,46.985]
    flagregions.append(flagregion)


    # South shore of Grays Harbor 1
    # Level 8 is 1/3 sec
    # Note that this region contains the fgmax_extent, but is larger
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_SG1'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 2  # Ruled Rectangle
    flagregion.spatial_region_file = os.path.abspath('RuledRectangle_SG1.data')
    flagregions.append(flagregion)

    x73=-124.06842593; x497=-123.98916667;
    slu_SG1=np.array([[-124.146667,46.8025,46.93],[x73,46.8025,46.93],\
                      [x497,46.835,46.93],\
                      [-123.9303,46.8565,46.93]])
    rr_SG1 = region_tools.RuledRectangle(slu=slu_SG1)
    rr_SG1.ixy = 'x'     #s refers to x, the longitude, lower and upper in lat y
    rr_SG1.method = 1  #connect linearly
    rr_SG1_name = 'RuledRectangle_SG1'
    rr_SG1.write(rr_SG1_name + '.data')
    ...
    if 1:  #Bridges in the 1/3" South Grays1 region
        gauges.append([63, -123.99925926, 46.89990741, 0, 1e+09])
        gauges.append([70, -123.95814815, 46.91768519, 0, 1e+09])
        gauges.append([73, -124.06842593, 46.86250000, 0, 1e+09])
        gauges.append([235, -124.09194444, 46.80990741, 0, 1e+09])
        gauges.append([424, -124.09194444, 46.81620370, 0, 1e+09])
        gauges.append([436, -124.09194444, 46.81166667, 0, 1e+09])
        gauges.append([446, -123.95018519, 46.86907407, 0, 1e+09])
        gauges.append([447, -123.94379630, 46.86583333, 0, 1e+09])
        gauges.append([448, -123.93166667, 46.85842593, 0, 1e+09])
        gauges.append([496, -124.07703704, 46.83611111, 0, 1e+09])
        gauges.append([497, -123.98916667, 46.83842593, 0, 1e+09])
        gauges.append([498, -123.95046296, 46.87444444, 0, 1e+09])
        gauges.append([499, -123.98166667, 46.90416667, 0, 1e+09])
        gauges.append([500, -123.95787037, 46.92092593, 0, 1e+09])
        gauges.append([531, -124.04546296, 46.89277778, 0, 1e+09])
        gauges.append([532, -124.03120370, 46.89759259, 0, 1e+09])
        gauges.append([545, -124.04777778, 46.88592593, 0, 1e+09])
        gauges.append([546, -124.03212963, 46.89277778, 0, 1e+09])
        gauges.append([548, -124.04574074, 46.89129630, 0, 1e+09])
