...
#------------------------------
    ...
    # Hoquiam 1, Important bridges, bit east of HS
    # Level 8 is 1/3 sec
    # Note that this region contains the fgmax_extent, but is larger
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_HOQ1'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-123.896,-123.873,46.958,47.0065]
    flagregions.append(flagregion)

    #G85a goes up river valley to Gauge 85
    #To get the river water, use with HOQ1
    # Level 8 is 1/3 sec
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_G85a'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-123.9245,-123.896,46.995394,47.0327]
    flagregions.append(flagregion)

    #G85b goes up river valley north of Gauge 85
    #To get the river water, use with HOQ1
    # Level 8 is 1/3 sec
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_G85b'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-123.9302,-123.9185,47.0327,47.0576]
    flagregions.append(flagregion)

    # G140 includes gauge 140, to be run with HOQ1
    # Level 8 is 1/3 sec
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_G140'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-123.9303,-123.896,46.986,46.995394,]
    flagregions.append(flagregion)
    ...
    if 1:
        #From HOQ1
        gauges.append([26, -123.87712963, 46.97537037, 0, 1e+09])
        gauges.append([88, -123.89509259, 46.99425926, 0, 1e+09])
        gauges.append([126, -123.88425926, 46.97944444, 0, 1e+09])
        gauges.append([359, -123.88212963, 47.00509259, 0, 1e+09])
        gauges.append([559, -123.87462963, 46.97175926, 0, 1e+09])
        gauges.append([560, -123.87648148, 46.97194444, 0, 1e+09])
    if 1:
        #From G85b
        gauges.append([139, -123.92527778, 47.05074074, 0, 1e+09])
        gauges.append([248, -123.92777778, 47.05703704, 0, 1e+09])
    if 1:
        #From G85a
        gauges.append([85, -123.92203704, 47.03101852, 0, 1e+09])

    if 1:
        #From G140
        gauges.append([140, -123.92148148, 46.98935185, 0, 1e+09])
