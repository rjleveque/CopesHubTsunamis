...
#------------------------------
    ...
    if clawdata.output_style==1:
        ...
        #clawdata.tfinal = 10.0     #test run for 10 seconds
    ...
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.245,-123.75, 46.855,47.06]
    flagregions.append(flagregion)

    # South shore of Grays Harbor 2
    # Level 8 is 1/3 sec
    # Note that this region contains the fgmax_extent, but is larger
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_SG2'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 2  # Ruled Rectangle
    flagregion.spatial_region_file = os.path.abspath('RuledRectangle_SG2.data')
    flagregions.append(flagregion)

    slu_SG2=np.array([[-123.9303,46.93,46.970], [-123.9081,46.93,46.970],[-123.849722,46.939,46.970],\
                      [-123.8188,46.9515,46.970]])
    rr_SG2 = region_tools.RuledRectangle(slu=slu_SG2)
    rr_SG2.ixy = 'x'     #s refers to x, the longitude, lower and upper in lat y
    rr_SG2.method = 1  #connect linearly
    rr_SG2_name = 'RuledRectangle_SG2'
    rr_SG2.write(rr_SG2_name + '.data')
    ...
    if 1:  #Bridges in the 1/3" South Grays2 region
        gauges.append([49, -123.87740741, 46.94009259, 0, 1e+09])
        gauges.append([50, -123.90805556, 46.93361111, 0, 1e+09])
        gauges.append([51, -123.89731481, 46.93583333, 0, 1e+09])
        gauges.append([52, -123.88768519, 46.93787037, 0, 1e+09])
        gauges.append([71, -123.85296296, 46.94342593, 0, 1e+09])
        gauges.append([135, -123.83833333, 46.94740741, 0, 1e+09])
        gauges.append([501, -123.90814815, 46.93092593, 0, 1e+09])
        gauges.append([502, -123.85157407, 46.95018519, 0, 1e+09])
        gauges.append([503, -123.84972222, 46.94064815, 0, 1e+09])
        gauges.append([504, -123.84250000, 46.95203704, 0, 1e+09])
        gauges.append([505, -123.84268519, 46.95222222, 0, 1e+09])
        gauges.append([506, -123.82250000, 46.95638889, 0, 1e+09])
        gauges.append([507, -123.82240741, 46.95620370, 0, 1e+09])
        gauges.append([508, -123.82064815, 46.95592593, 0, 1e+09])
