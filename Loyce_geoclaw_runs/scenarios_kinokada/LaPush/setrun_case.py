...
#------------------------------
    ...
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-126.6,-124.175,47.1,48.62]
    flagregions.append(flagregion)
    ...
    gauges_region = [-125.9,-124.375,47.55,48.375]
    ...
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.75,-124.5,47.83,48.0]
    flagregions.append(flagregion)
    ...
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.68,-124.53, 47.88,47.95]
    flagregions.append(flagregion)

    # LaPush 1/3sec region 
    # Level 8 is 1/3 sec
    flagregion = FlagRegion(num_dim=2)
    flagregion.name = 'Region_13sec_LaPush'
    flagregion.minlevel = 8
    flagregion.maxlevel = 8
    flagregion.t1 = 0.
    flagregion.t2 = 1e9
    flagregion.spatial_region_type = 1  # Rectangle
    flagregion.spatial_region = [-124.655,-124.565,47.895,47.93]
    flagregions.append(flagregion)
    ...
    gauges = []
    ### These are LaPush centered gauges
    gauges.append([1, -124.63675926, 47.91129630, 0, 1e+09])
    gauges.append([2, -124.63564815, 47.91101852, 0, 1e+09])
    gauges.append([3, -124.63638889, 47.90962963, 0, 1e+09])
    gauges.append([4, -124.63462963, 47.91314815, 0, 1e+09])
    gauges.append([5, -124.63824074, 47.91712963, 0, 1e+09])
    gauges.append([6, -124.63898148, 47.90814815, 0, 1e+09])
    gauges.append([7, -124.63203704, 47.90601852, 0, 1e+09])
    gauges.append([8, -124.61055556, 47.89129630, 0, 1e+09])

    if 0: #Hoh centered in 1/3" cells
        #Bureau Indian Affairs
        gauges.append([1, -124.41925926, 47.74370370, 0, 1e+09])

        #Hoh River mouth
        gauges.append([2, -124.43370370, 47.74842593, 0, 1e+09])

        #Lower Hoh Rd - River entrance
        gauges.append([3, -124.43231481, 47.74842593, 0, 1e+09])

        #2455 Lower Hoh Rd
        gauges.append([4, -124.42222222, 47.74500000, 0, 1e+09])

        #Lower Hoh Rd-Chalatt Loop
        gauges.append([5, -124.41648148, 47.74250000, 0, 1e+09])

        #Chalaat Loop
        gauges.append([6, -124.41388889, 47.74240741, 0, 1e+09])

        #Chief Klia Wellness Center
        gauges.append([7, -124.41314815, 47.74101852, 0, 1e+09])

        #Oil City Trailhead
        gauges.append([8, -124.40861111, 47.74935185, 0, 1e+09])

        #9772 Oil City Rd
        gauges.append([9, -124.40407407, 47.74990741, 0, 1e+09])

        #Oil City Road - Fossil Creek
        gauges.append([10, -124.40509259, 47.74972222, 0, 1e+09])

        #National Playground
        gauges.append([11, -124.41509259, 47.74185185, 0, 1e+09])

        #Pacific NW Trail
        gauges.append([12, -124.42407407, 47.74953704, 0, 1e+09])

        #Lower Hoh Rd East
        gauges.append([13, -124.40370370, 47.74064815, 0, 1e+09])

        #West Shore
        gauges.append([14, -124.42787037, 47.74287037, 0, 1e+09])

        #US 101 at Ruby Beach
        gauges.append([15, -124.41111111, 47.71000000, 0, 1e+09])

        #US 101 at Kalaloch
        gauges.append([16, -124.37157407, 47.60712963, 0, 1e+09])
    ...
    #LaPush
    #fgmax_extent=[-124.435,-124.4025,47.74,47.75], not doing fgmax
    if 0:
    ...
    if 1:
        ...
        #This is the region where 1/3 computation was for LaPush
        #flagregion.spatial_region = [-124.655,-124.565,47.895,47.93]
        ...
        fgout.nx = 973
        fgout.ny = 379
        fgout.x1 = -124.655 + one_sixth
        fgout.x2 = -124.565 - one_sixth
        fgout.y1 = 47.895 + one_sixth
        fgout.y2 = 47.93 - one_sixth
        fgout.tstart = 0.0
        fgout.tend = 2*3600
