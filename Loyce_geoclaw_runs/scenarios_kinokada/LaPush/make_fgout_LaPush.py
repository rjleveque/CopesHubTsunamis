...
Specific to LaPush, for other sites it is necessary to change the axis
...
def make_anim(outdir, plotdir, location, event):
    ...
    ax.set_aspect(1/cos(47.91*pi/180))
    ...
    # G3 is Gauge 3 at the Quileute Tribal Center Center
    yt1 = 47.90962963; Ttitle1 = 'G3'
    
    #west, east limits of the transect
    x1trans, x2trans = fgout1.extent_edges[:2]
    
    plot([x1trans,x2trans], [yt1,yt1],'k-',linewidth=0.8)
    text(-124.6549,yt1+0.0005,'Transect %s' % Ttitle1)
    ...
    def annotate_transect(axtrans):
        dxkm = 1
        dxlong = dxkm/(111.* cos(pi*47.91/180))
        axtrans.plot([-124.6549,-124.6549+dxlong], [-12,-12],'k')
        axtrans.text(-124.6549+dxlong/2, -13, '%i km' % dxkm, \
                     ha='center',va='top')
    
        xG3 = -124.63638889; yG3= 47.90962963;
        axtrans.plot([xG3,xG3],[-20,0],'k--')
        axtrans.text(-124.636, -13, ' G3')
        axtrans.grid(True)
