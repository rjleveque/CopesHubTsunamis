...
def report(outdir,plotdir,location,event,dtopofile,run_name):
    ...
    for gaugeno in gaugeno_list:
        ...
        if 1:
            fprint(' Initial bathymetry at the gauge before quake in m was: %.3f ' %B0 )
            fprint(' Final bathymetry at the gauge right after quake in meters  was: %.3f '\
                   %Bpost_quake_finest )
            fprint(' Final bathymetry at the gauge at end of job run in m was: %.3f' %B[-1])
            fprint(' ')
