...
def adjust(z_desired, z_edge, dz, verbose=False):
    ...
    print('+++ (z_desired-z_edge - 0.5*dz)/dz = ',(z_desired-z_edge - 0.5*dz)/dz)
    if verbose:
...
def center_all_gaugedata(rundata, dx, dy, verbose=False):
    """
    Function to be called from setrun after setting rundata.gaugedata,
    ...
    x_centered, y_centered = adjust_xy(x_desired, y_desired,
                                       x_edge, y_edge, dx, dy,
                                       verbose=verbose)

    # reset the x,y values for each gauge:

    for k,gauge in enumerate(rundata.gaugedata.gauges):
        #gauge[1] = float(x_centered[k])
        #gauge[2] = float(y_centered[k])
        gauge[1] = x_centered[k]
        gauge[2] = y_centered[k]
    print('*** Shifted gauges if necessary to center in cells')
